...
import type { TireReplacementLog, Vehicle } from '../models';

type Props = NativeStackScreenProps<RootStackParamList, 'TireReplacementHistory'>;

export default function TireReplacementHistoryScreen({ route }: Props) {
  const { vehicleId } = route.params;
  const [logs, setLogs] = React.useState<TireReplacementLog[]>([]);
    ...
    const load = async () => {
      try {
        const [list, v] = await Promise.all([
          repo.listTireReplacementLogs(vehicleId),
  ...
  const renderHeader = () => (
    <View style={[styles.row, styles.headerRow]}>
      ...
      <Text style={[styles.cell, styles.headerCell]}>Neumático</Text>
    </View>
  );

  return (
    <View style={styles.container}>
      <Text style={styles.hint}>Histórico de sustitución de neumáticos{vehicle ? ` · ${vehicle.make} ${vehicle.model}` : ''}</Text>
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <View style={[styles.row, index % 2 ? styles.altRow : null]}>
              ...
              <Text style={styles.cell}>{item.tireType || '-'}</Text>
            </View>
          )}
